...
```mermaid
flowchart TD
    A[카카오 로그인] --> B{로그인 상태 확인}
    B -->|비회원| C[로그인 페이지]
    B -->|회원| D[메인 페이지]
    C --> E[카카오톡 SSO]
    E --> F{신규/기존 사용자}
    F -->|신규| G[데이터베이스에 정보 입력]
    F -->|기존| H[로그인 완료]
    G --> H
    H --> I[헤더 메뉴 변경]
    I --> D
...
| 상태       | 설명                              | 가능한 액션               |
|----------|---------------------------------|----------------------|
| **모집중**  | 신청 접수 중인 모임                     | 신청, 신청취소, 승인, 거절, 댓글 |
| **모집마감** | 정원이 찬 모임                        | 신청, 신청취소, 거절, 댓글     |
| **진행중**  | 모임 시간이 지난 이후 진행 중인 모임           | 모임 종료, 댓글            |
| **완료**   | 모임 종료                           | 피드백 작성, 댓글           |
| **취소**   | 모임 시간이 지났지만 1명만 있는 모임은 자동적으로 취소 | 마이페이지에서 조회 가능        |
...
| v1.0.2 | 2025.09.22 | 화면 구조 정의, 핵심 User Flow, 상세 화면별 Flow, 상태 관리 업데이트 | 강관주 |
---
